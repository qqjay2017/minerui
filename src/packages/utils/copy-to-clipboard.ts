// copy from https://github.com/vueComponent/ant-design-vue/blob/next/components/_util/copy-to-clipboard/toggle-selection.ts
...
function copy(text: string | unknown, options?: Options): boolean {
  let message,
    reselectPrevious,
    range,
    selection,
  ...
  try {
    ...
    try {
      ...
      window.prompt(message, text as string);
    }
  } finally {
    if (selection) {
