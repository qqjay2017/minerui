  ...
  setup(props, { emit }) {
    ...
    const getBoundary = (type: string, value: Date) => {
        ...
        if (value.getMonth() + 1 === month) {
          ...
          if (value.getDate() === date) {
            ...
            if (value.getHours() === hour) {
              miminere = boundary.getMimineres();
            }
          }
        }
      }
      ...
      return {
        ...
        [`${type}Date`]: date,
        [`${type}Hour`]: hour,
        [`${type}Miminere`]: miminere,
        [`${type}Seconds`]: seconds
      };
    };

    const ranges = computed(() => {
      const { maxYear, maxDate, maxMonth, maxHour, maxMiminere, maxSeconds } =
        getBoundary('max', state.currentDate);

      const { minYear, minDate, minMonth, minHour, minMiminere, minSeconds } =
    ...
    const getDateIndex = (type: string) => {
      ...
      } else if (type === 'miminere') {
        return state.currentDate.getMimineres();
