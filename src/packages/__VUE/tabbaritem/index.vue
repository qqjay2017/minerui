<template>
  <div
    class="miner-tabbar-item"
    :class="{ 'miner-tabbar-item__icon--unactive': state.active != state.index }"
    :style="{
      color:
  ...
  >
    <view class="miner-tabbar-item_icon-box">
      <view
        class="miner-tabbar-item_icon-box_tips miner-tabbar-item_icon-box_num"
        v-if="num && num <= 99"
      >
        {{ num }}
      </view>
      <view
        class="miner-tabbar-item_icon-box_tips miner-tabbar-item_icon-box_nums"
        v-else-if="num && num > 100"
        >{{ '99+' }}</view
      >
      <view v-if="icon">
        <miner-icon
          class="miner-tabbar-item_icon-box_icon"
          :size="state.size"
          :name="icon"
      ...
      <view
        :class="[
          'miner-tabbar-item_icon-box_nav-word',
          { 'miner-tabbar-item_icon-box_big-word': !icon && !activeImg }
        ]"
        >{{ tabTitle }}</view
      >
    </view>
