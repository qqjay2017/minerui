<template>
  <view :class="classes">
    <miner-popup v-model:visible="showMask"></miner-popup>
    <view class="miner-menu-title" @click="handleMenuPanel">
      <view class="title-name" v-html="menuTitle"></view>
      <miner-icon class-prefix="icon"></miner-icon>
    </view>

    <view class="miner-menu-panel" :style="`max-height:${maxHeight}px`">
      <view
...
export default create({
  ...
  setup(props, { emit }) {
    ...
    const handleShowAndHide = (event: any) => {
      const menuBox = document.querySelectorAll('.miner-menu-active')[0];
