...
import { Uploader,Icon } from '@minerui/minerui';
...
``` html
<miner-uploader url="http://服务器地址" capture></miner-uploader>
```
### 限制上传数量5个
...
``` html
<miner-uploader url="http://服务器地址" multiple :maximize="1024 * 50" :before-upload="beforeUpload" @oversize="onOversize"></miner-uploader>
```
...
``` html
<miner-uploader url="http://服务器地址" :data="formData" :headers="formData" :with-Credentials="true"></miner-uploader>
```
