    ...
    >
      <view :class="['tab-title', iconType, 'tab-title-scroll']" ref="navlist">
        <view
          :class="[
            'tab-title-box',
            { 'miner-tab-active': activeIndex == index },
      ...
      <view v-if="titles.length > 0" class="tab-swiper" ref="mineruiSwiper">
...
export default create({
  ...
  setup(props, ctx) {
    ...
    function switchTitle(index: number) {
      activeIndex.value = index;
      centerTitle(index);
      console.log(mineruiSwiper.value);
      mineruiSwiper.value.to(index);
    }
    function initTitle() {
      titles.length = 0;
