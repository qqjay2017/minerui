...
test('base', () => {
  ...
  const input: any = wrapper.find('.miner-input-real');
...
test('base length and error', () => {
  ...
  const input = wrapper.find('.miner-input-real');
  expect(input.exists()).toBe(true);
  // expect(input.attributes("maxlength")).toBe("4");
  const error = wrapper.find('.miner-shortpsd-error');
  expect(error.exists()).toBe(true);
  expect(error.text()).toBe('错误信息');
  const cancle = wrapper.find('.miner-shortpsd-cancle');
  expect(cancle.exists()).toBe(true);
  const sure = wrapper.find('.miner-shortpsd-sure');
