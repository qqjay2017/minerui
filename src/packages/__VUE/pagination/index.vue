<template>
  <view class="miner-pagination">
    <view
      :class="['miner-pagination-prev', mode == 'multi' ? '' : 'simple-border', modelValue == 1 ? 'disabled' : '']"
    ...
    <view class="miner-pagination-contain" v-if="mode == 'multi'">
      <view
        v-for="(item, index) of pages"
        :key="index + 'pagination'"
        :class="['miner-pagination-item', item.active ? 'active' : '']"
    ...
    <view class="miner-pagination-contain" v-if="mode == 'simple'">
      <view class="miner-pagination-simple">{{ modelValue }}/{{ countRef }}</view>
    </view>
    <view
      :class="['miner-pagination-next', modelValue >= countRef ? 'disabled' : '']"
      @click="select(modelValue + 1, true)"
    >
      <slot name="next-text">
        {{ nextText }}
